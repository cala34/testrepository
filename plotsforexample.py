...
'''
script to produce plots in section 5.2 for Gaussian Kernel
'''
...
def grid(num_translates, kernel):
    ...
    # np.random.seed() sets the seed to the current system time
    # we chose a seed in order to reproduce the Thesis plots
    np.random.seed(12)
...
if __name__ == "__main__":
    ...
    # training with tolerance

    train_param = {}
    train_param['kernel'] = kernel
    train_param['max_iterations'] = 190
    ...
    # training until max expansion size
    ...
    print("min residual of", np.min(results['max_residual']), "at iteration", np.argmin(results['max_residual']))
    print("min power of", np.min(results['max_power_fct']), "at iteration", np.argmin(results['max_power_fct']))
    print("min rkhs error of", np.min(results['rkhs_error']), "at iteration", np.argmin(results['rkhs_error']))
